...
# setup.sh - 'jd agent-setup', the prompt for your agent
#
...
_jd_t_run jd agent-setup
_jd_t_status 'no config: jd agent-setup exit status' 0
...
_jd_t_run jd agent-setup --help
_jd_t_status 'help: exit status' 0
_jd_t_contains 'help: prints the usage line' 'usage: <system> agent-setup' "$JD_T_OUT"
_jd_t_contains 'help: names pbcopy' 'pbcopy' "$JD_T_OUT"

_jd_t_run jd agent-setup now
_jd_t_status 'extra word: is an error' 1
_jd_t_contains 'extra word: says agent-setup takes nothing' \
  "'jd agent-setup' takes nothing after it" "$JD_T_ERR"
_jd_t_eq 'extra word: nothing on stdout' '' "$JD_T_OUT"

_jd_t_run jd help
_jd_t_contains 'jd help: names agent-setup' '<system> agent-setup' "$JD_T_OUT"

# Every command that needs the config says the same thing, and names
# the way out.

_jd_t_run jd 11.11
_jd_t_contains 'no config: a move names jd agent-setup' 'jd agent-setup' "$JD_T_ERR"

_jd_t_run jd beta
_jd_t_contains 'no config: jd beta names jd agent-setup' 'jd agent-setup' "$JD_T_ERR"

_jd_t_run jd new id 21 A title
_jd_t_status 'no config: jd new is an error' 1
_jd_t_contains 'no config: jd new names jd agent-setup' 'jd agent-setup' "$JD_T_ERR"
...
_jd_t_run jd agent-setup
_jd_t_status 'config exists: jd agent-setup still works' 0
